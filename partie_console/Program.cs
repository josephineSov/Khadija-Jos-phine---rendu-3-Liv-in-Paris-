...
namespace psi_joséphine
{
    class Program
    {
        ...
        static void CreerNouveauCompte()
        {
            ...
            bool champsValides = !string.IsNullOrEmpty(nouvelUtilisateur.Prenom) &&
                               !string.IsNullOrEmpty(nouvelUtilisateur.Nom) &&
                               !string.IsNullOrEmpty(nouvelUtilisateur.Email) &&
                               !string.IsNullOrEmpty(nouvelUtilisateur.MotDePasse) &&
                               !string.IsNullOrEmpty(nouvelUtilisateur.Role);
        ...
        static void GererMenuConnecte(string choix, ref Personne utilisateurConnecte, GrapheUtilisateur grapheUtilisateur)
        {
            ...
            switch (choix)
            {
                ...
                case "5": // Ajouter plat ou déconnexion
                    if (estCuisinier)
                    {
                        AjouterPlat();
                    }
        ...
        static void AfficherCommandesUtilisateur(Personne utilisateur)
        {
            try
            {
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    ...
                    string requete = "SELECT c.*, p.nom as nom_plat, p.prix as prix_plat, lc.quantite " +
                                   "FROM commande c " +
                                   "LEFT JOIN lignecommande lc ON c.id = lc.commande_id " +
                                   "LEFT JOIN plat p ON lc.plat_id = p.id " +
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (!reader.IsDBNull(reader.GetOrdinal("nom_plat")))
                        {
                            ...
                            Console.WriteLine($"  • {nomPlat} x{quantite} ({prixPlat}€/unité)");
                        }
                    }
        ...
        {
            // TODO: Implémenter l'ajout de plat
        ...
        static void AfficherLivraisonsParCuisinier()
        {
            try
            {
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    con.Open();
                    var command = con.CreateCommand();
                    string requete = "SELECT u.prenom, u.nom, COUNT(l.id_livraison) as nombre_livraisons " +
                                   "FROM utilisateurs u " +
                                   "LEFT JOIN livraison l ON u.id = l.cuisinier_id " +
                                   "WHERE u.role IN ('cuisinier', 'client_cuisinier') " +
        ...
        static void AfficherCommandesParPeriode()
        {
            try
            {
                ...
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    con.Open();
                    var command = con.CreateCommand();
                    string requete = "SELECT DATE(date_creation) as date, COUNT(*) as nombre_commandes " +
                                   "FROM commande " +
                                   "WHERE DATE(date_creation) BETWEEN @dateDebut AND @dateFin " +
        ...
        static void AfficherMoyennePrixCommandes()
        {
            try
            {
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    con.Open();
                    var command = con.CreateCommand();
                    string requete = "SELECT AVG(total) as moyenne_prix " +
                                   "FROM commande " +
                                   "WHERE statut != 'Annulé'";
                    command.CommandText = requete;
                    ...
                    if (reader.Read())
                    {
                        ...
                        Console.WriteLine($"\nMoyenne des prix des commandes: {moyennePrix:F2}€");
                    }
                }
            }
            catch (Exception ex)
        ...
        static void AfficherMoyenneComptesClients()
        {
            try
            {
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    con.Open();
                    var command = con.CreateCommand();
                    string requete = "SELECT u.id, u.prenom, u.nom, AVG(c.total) as moyenne_depenses " +
                                   "FROM utilisateurs u " +
                                   "LEFT JOIN commande c ON u.id = c.client_id " +
                                   "WHERE u.role IN ('client', 'client_cuisinier') " +
                                   "GROUP BY u.id, u.prenom, u.nom " +
                                   "ORDER BY moyenne_depenses DESC";
                    command.CommandText = requete;
                    ...
                    while (reader.Read())
                    {
                        decimal moyenneDepenses = reader["moyenne_depenses"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["moyenne_depenses"]);
                        Console.WriteLine($"{reader["prenom"]} {reader["nom"]}: {moyenneDepenses:F2}€");
                    }
                }
            }
            catch (Exception ex)
        ...
        static void AfficherCommandesParNationaliteEtPeriode()
        {
            try
            {
                ...
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    con.Open();
                    var command = con.CreateCommand();
                    string requete = "SELECT n.nationalite_cuisine, COUNT(*) as nombre_commandes " +
                                   "FROM commande c " +
                                   "JOIN commande_nourriture cn ON c.id = cn.commande_id " +
                                   "JOIN Nourriture n ON cn.nourriture_id = n.id_Nourriture " +
                                   "WHERE DATE(c.date_creation) BETWEEN @dateDebut AND @dateFin " +
                                   "GROUP BY n.nationalite_cuisine " +
        ...
        static void NoterCuisinier(Personne utilisateur)
        {
            try
            {
                ...
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    con.Open();
                    var command = con.CreateCommand();
                    string requete = "SELECT c.id, c.date_creation, u.prenom, u.nom " +
                                   "FROM commande c " +
                                   "JOIN utilisateurs u ON c.cuisinier_id = u.id " +
                                   "WHERE c.client_id = @userId AND c.statut = 'Livré' " +
                                   "ORDER BY c.date_creation DESC";
                    command.CommandText = requete;
                    command.Parameters.AddWithValue("@userId", utilisateur.Id);
                    ...
                    requete = "SELECT c.id, u.prenom, u.nom " +
                             "FROM commande c " +
                             "JOIN utilisateurs u ON c.cuisinier_id = u.id " +
                    ...
                    command = con.CreateCommand();
                    requete = "INSERT INTO Notes (commentaire_note, date_avis, note_retour, commande_id) " +
                             "VALUES (@commentaire, NOW(), @note, @commandeId)";
                    command.CommandText = requete;
        ...
        static void GererCommandesCuisinier(Personne cuisinier)
        {
            try
            {
                using (var con = new MySql.Data.MySqlClient.MySqlConnection(BaseUtilisateur.connexionString))
                {
                    con.Open();
                    var command = con.CreateCommand();
                    string requete = "SELECT c.id, c.date_creation, c.statut, u.prenom, u.nom, u.adresse " +
                                   "FROM commande c " +
                    ...
                    command = con.CreateCommand();
                    requete = "SELECT c.id, c.statut, u.adresse as adresse_client " +
                             "FROM commande c " +
                             "JOIN utilisateurs u ON c.client_id = u.id " +
                             "WHERE c.id = @commandeId AND c.cuisinier_id = @cuisinierId";
                    command.CommandText = requete;
                    command.Parameters.AddWithValue("@commandeId", commandeId);
                    command.Parameters.AddWithValue("@cuisinierId", cuisinier.Id);
                    ...
                    if (nouveauStatut == "En cours" && statutActuel != "En cours")
                    {
                        ...
                        requete = "INSERT INTO livraison (adresse_depart, adresse_arrivee, date_livraison, statut_livraison, heure_disponible, zone_geographique, cuisinier_id) " +
                                 "VALUES (@adresseDepart, @adresseArrivee, NOW(), 'En attente', TIME(NOW()), 'Paris', @cuisinierId)";
                        command.CommandText = requete;
