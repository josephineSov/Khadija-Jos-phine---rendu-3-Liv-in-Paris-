...
namespace psi_joséphine
{
    ...
    public class Lien
    {
        ...
        {
            const double R = 6371;
            double lat1 = a.Lat * Math.PI / 180; // converti en radians 
            double lon1 = a.Lon * Math.PI / 180;
            double lat2 = b.Lat * Math.PI / 180;
            double lon2 = b.Lon * Math.PI / 180;

            double dLat = lat2 - lat1; // difference angulaire 
            double dLon = lon2 - lon1;

            double haversine = Math.Pow(Math.Sin(dLat / 2), 2) +
                               Math.Cos(lat1) * Math.Cos(lat2) * Math.Pow(Math.Sin(dLon / 2), 2); //  permet de calculer la distance en ligne droite entre deux points sur une sphère.
